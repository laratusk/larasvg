...
> **Note:** `try.php` is tracked in git for convenience but is **not bundled in Composer releases** (`archive.exclude` in `composer.json`). It is purely a development tool and has no impact on package consumers.
