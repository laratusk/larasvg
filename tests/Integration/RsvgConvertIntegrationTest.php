...
/**
 * Integration tests that run real rsvg-convert commands.
 * Requires rsvg-convert (librsvg) to be installed on the system.
 */
class RsvgConvertIntegrationTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->app['config']->set('svg-converter.providers.rsvg-convert.binary', env('RSVG_CONVERT_PATH', 'rsvg-convert'));
        $this->app['config']->set('svg-converter.providers.rsvg-convert.timeout', 60);

        $binary = $this->app['config']->get('svg-converter.providers.rsvg-convert.binary', 'rsvg-convert');

        if (! file_exists($binary) && ! $this->commandExists($binary)) {
            $this->markTestSkipped("rsvg-convert binary not found at: {$binary}");
        }
    ...
    #[Test]
    public function it_gets_real_rsvg_convert_version(): void
    {
        $version = SvgConverter::using('rsvg-convert')->version('rsvg-convert');
    ...
    {
        $output = SvgConverter::using('rsvg-convert')
            ->open($this->testSvg)
            ->setDimensions(64, 64)
            ->toStdout('png');
    ...
    {
        $result = SvgConverter::using('rsvg-convert')
            ->open($this->testSvg)
            ->setFormat('png')
    ...
    {
        $exportName = sys_get_temp_dir().'/'.uniqid('rsvg_convert_explicit_').'.png';
    ...
    public function it_throws_on_invalid_input_file(): void
    {
        ...
        SvgConverter::using('rsvg-convert')->open('/nonexistent/file.svg');
    }
    ...
    {
        $path = sys_get_temp_dir().'/'.uniqid('rsvg_convert_test_').'.'.$extension;
